...
EN.601.507, advised by Professor Tom Lippincott).}}
...
\texttt{pr\_curve()}. The latter three implement the evaluation
...
produced. We additionally report a precision-recall curve produced
by sweeping the threshold over the distinct observed score values
for each model (capped at 200 quantile-thinned points so densely
populated distributions stay readable). A fixed grid in [0, 1] is
the conventional choice but loses resolution when scores cluster
bimodally or in a narrow band, which collapses the resulting curve
to a near-straight diagonal that misrepresents the precision-recall
trade-off. Sweeping at observed values keeps every operating point
the model can actually express.
...
Precision-recall curves swept across each model's observed score
values are shown in Figure \ref{fig:pr}. The curves are useful for
...
Each curve is one model swept across the distinct observed score
values for that model, with stochastic models averaged over three
seeds. ExactMatch is plotted as a single marker because it produces
a binary score and cannot interpolate a precision-recall trade-off.}
\label{fig:pr}
\end{figure*}
...
This work was carried out under the supervision of Professor Tom Lippincott
