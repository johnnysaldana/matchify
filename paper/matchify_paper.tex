...
A Comparative Study Across Four Benchmark Datasets}

\author[1]{Johnny Saldana}
\affil[1]{Department of Computer Science, Johns Hopkins University \\
\texttt{jsaldan2@jhu.edu}}

\date{EN.601.507 Applied Entity Resolution and Deduplication \\
Spring 2023 \\
...
We adopt a held-out group split protocol so that supervised models
never observe the group identifiers used at evaluation time, and we
report mean reciprocal rank and threshold-swept precision-recall
curves with multi-seed standard deviations on the stochastic models.
The empirical findings are consistent with the literature in three of
four configurations and surprising in one. On the cross-catalog
...
exact-match baseline that returns the rules-of-the-game lower bound.
...
chosen to fit a single-laptop runtime budget, and Section
\ref{sec:limitations} discusses the implications.
...
\begin{table}[t]
\centering
\caption{Amazon-Google Products. 500 records, 142 test rows.}
\label{tab:ag}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
ExactMatch & 0.239 & 1.000 & 0.200 & 0.333 \\
FlexMatch  & 0.465 & 0.095 & 0.881 & 0.172 \\
MLP        & 0.602 \scriptsize{$\pm$ 0.048} & 0.277 & 0.986 & \textbf{0.431} \scriptsize{$\pm$ 0.057} \\
Bert       & 0.639 & 0.127 & 0.990 & 0.224 \\
Siamese    & \textbf{0.667} \scriptsize{$\pm$ 0.019} & 0.095 & 1.000 & 0.174 \scriptsize{$\pm$ 0.008} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Abt-Buy. 500 records, 151 test rows.}
\label{tab:ab}
\small
\begin{tabular}{lcccc}
\toprule
...
FlexMatch  & 0.169 & 0.022 & 0.303 & 0.042 \\
MLP        & \textbf{0.714} \scriptsize{$\pm$ 0.078} & 0.179 & 0.967 & \textbf{0.302} \scriptsize{$\pm$ 0.024} \\
Bert       & 0.516 & 0.063 & 1.000 & 0.118 \\
Siamese    & 0.638 \scriptsize{$\pm$ 0.073} & 0.044 & 1.000 & 0.085 \scriptsize{$\pm$ 0.018} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{DBLP-ACM. 500 records, 150 test rows.}
\label{tab:dblp}
\small
\begin{tabular}{lcccc}
\toprule
...
\midrule
ExactMatch & 0.000 & 0.000 & 0.000 & 0.000 \\
FlexMatch  & 0.933 & 0.306 & 1.000 & 0.469 \\
MLP        & 0.920 \scriptsize{$\pm$ 0.013} & \textbf{1.000} & \textbf{1.000} & \textbf{1.000} \\
Bert       & 0.933 & 0.664 & 1.000 & 0.798 \\
Siamese    & 0.920 \scriptsize{$\pm$ 0.013} & 0.491 & 1.000 & 0.659 \scriptsize{$\pm$ 0.019} \\
\bottomrule
\end{tabular}
\end{table}
...
Model & MRR & Prec & Rec & F1 \\
\midrule
ExactMatch & 0.476 & 1.000 & 0.411 & 0.583 \\
FlexMatch  & 0.929 & 0.307 & 1.000 & 0.470 \\
MLP        & 0.919 \scriptsize{$\pm$ 0.027} & 0.909 & 1.000 & \textbf{0.952} \scriptsize{$\pm$ 0.012} \\
Bert       & \textbf{0.933} & 0.197 & 1.000 & 0.329 \\
Siamese    & 0.917 \scriptsize{$\pm$ 0.021} & 0.608 & 1.000 & 0.756 \scriptsize{$\pm$ 0.028} \\
\bottomrule
\end{tabular}
\end{table}
...
the surface forms diverge. The per-seed variance on Siamese (0.019) is
small enough that the four-MRR-point lead over BERT is unlikely to be
seed noise.
...
were on Amazon-Google, suggesting that pair sampling matters more when
the underlying training set is smaller. Second, Abt and Buy describe
products in radically different registers, with Abt offering long
marketing-prose descriptions and Buy offering terse listings. This
kind of cross-domain language gap is exactly the case that generic
semantic similarity, as offered by the pretrained sentence-transformer,
fails to bridge. The MLP, by contrast, learns on the labeled pairs
which similarity dimensions actually carry signal, and it can
downweight the prose-versus-terse mismatch in favor of correlations on
name and price. Third, the Siamese model does help over pretrained
BERT (0.64 versus 0.52) but does not catch up to MLP. We interpret
this as evidence that on a small, noisy benchmark, the encoder
fine-tuning has too little supervision to specialize the embedding
geometry usefully.
...
achieving perfect P=R=F1=1.0 and the cosine-similarity models trailing
because their score distributions place too many non-matching pairs
above 0.5 at inference time. This is not a failure of ranking but of
...
compared to this package's best F1.}
\label{tab:baselines}
\small
\begin{tabular}{lccc}
\toprule
...
deferred. The relative ordering across models is unlikely to invert on
full data, but absolute MRR and F1 levels would change.

The Magellan baseline was not re-run because
\texttt{py\_entitymatching} does not install on the Python 3.10+
environment in which the rest of the package runs. The DeepMatcher and
Ditto numbers cited in Section \ref{sec:baselines} are from those
papers' published evaluations under their own protocols.
...
reproducible, the resulting corpus is not standardized and its results
should be interpreted as a controlled case study rather than a
benchmark.
...
approximately 25 minutes on an Apple M-series MacBook.
